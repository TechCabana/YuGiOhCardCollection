...
A lightweight static site that displays a personal Yu-Gi-Oh! card collection. It
is plain HTML, CSS and ES modules served from GitHub Pages, with no framework, no
build step and nothing running on a server.
...
> Screenshot: pending. It goes in once the design-token and card-image work
> lands, so the image does not go stale immediately.
...
The browser only ever fetches the static `data/cards.json`, and never talks to
Airtable itself.

Why build time and not client side: Airtable has no read-only public API key. A
personal access token shipped in client-side JavaScript on a public repo grants
read and write on the base to anyone who views source. Fetching at build time
keeps the token in GitHub Secrets and ships a plain file.
...
Opening the page straight from disk does not work. `data.js` uses `fetch()` to
load `data/cards.json`, and `file://` requests fail CORS, so serve the directory
over HTTP:
...
Then open the printed URL. Viewing the site needs no `npm install`: the only
dependency is Vitest, and that is there for the tests.
...
2. Fill in the fields you own. Everything else is fetched:
   - `Serial`: the set code, for example `SDJ-017`. This is the lookup key.
   - `Quantity`: how many copies you own.
   - `Condition`: your grading.
   - `IsFirstEdition`: optional. Whether the copy is 1st Edition. A 1st Edition
     and an Unlimited copy share the same set code, and neither YGOPRODeck
     endpoint reports edition, so this cannot be derived. Only you can set it,
     and a sync will never touch it. It is not published to `data/cards.json`
     yet.
...
automatically, because Airtable's field-update endpoint accepts only `name` and
...
sync and commit still run, and the job fails at the end so the problem is visible
without holding up good data.
...
Or use the `Actions → Process Data → Run workflow` button. It also runs daily at
...
node scripts/enrich-airtable.mjs             # LIVE, sends real PATCH requests
...
a passcode with different serials are both kept, because deduplicating would lose
a printing from the collection.
...
| `passcode` | string | 8 digits, stored as text, because leading zeros are significant. Drives the card image URL. |
...
CI reads these from GitHub Secrets. They are not in `.env` for CI purposes, and
are only mirrored locally when a sync needs to be run by hand.
